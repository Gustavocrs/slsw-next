...
export default function LoreTab({ scenarioData, onUpdate, loading }) {
  ...
  const SHOW_RESTORE_BUTTON = false; // Mudar para true para mostrar o botão
  ...
  const handleAddSection = () => {
    ...
    setEditedContent(newSection.content);
    setIsDirty(false);
    setViewMode("edit");
  };
  ...
  const handleConfirmDelete = () => {
    ...
    setLoreSections(updated);
    onUpdate("loreSections", updated);
    if (selectedSection?.id === sectionToDelete) {
      setSelectedSection(null);
      setEditedContent("");
    }
  ...
  const handleRestoreDefault = async () => {
    if (
      !window.confirm(
        "Restaurar todas as seções padrão do manual? Isso substituirá o conteúdo atual.",
      )
    )
      return;
    ...
    try {
      ...
      if (!scenarioId) {
        alert("ID do cenário não encontrado");
        return;
      }
      ...
      if (result.success) {
        setLoreSections(result.data);
        onUpdate("loreSections", result.data);
        setSelectedSection(null);
        setEditedContent("");
        alert("Seções restauradas com sucesso!");
      } else {
        alert("Erro: " + result.error);
      }
    } catch (error) {
      console.error("Erro ao restaurar:", error);
      alert("Erro ao.restaurar seções");
    }
  };
          ...
          <Box sx={{ display: "flex", gap: 1 }}>
            {SHOW_RESTORE_BUTTON && (
              <Button
                ...
                disabled={loading}
                color="warning"
                title="Restaurar seções padrão do manual"
              >
                Restaurar
              </Button>
            )}
