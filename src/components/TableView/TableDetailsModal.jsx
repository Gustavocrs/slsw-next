...
import {clearScenarioCache} from "@/scenarios";
...
function TableDetailsModal() {
  ...
  const [scenarioDeleteDialogOpen, setScenarioDeleteDialogOpen] =
    useState(false);
  ...
  const [scenarioAdminInitialId, setScenarioAdminInitialId] = useState("");
  ...
  useEffect(() => {
    if (selectedTable) {
      ...
      setScenarioId(selectedTable.scenarioId || "");
      setIsPrivate(selectedTable.isPrivate ?? true);
      setInvites(selectedTable.invites || []);
      setExternalLink(selectedTable.externalLink || "");
      setIsEditing(false);

      // Validar se o cenário ainda existe
      const validateScenario = async () => {
        if (
          selectedTable.scenarioId &&
          selectedTable.scenarioId !== "solo-leveling"
        ) {
          try {
            const exists = await ScenarioService.getScenarioById(
              selectedTable.scenarioId,
            );
            if (!exists) {
              showNotification(
                `⚠️ O cenário "${selectedTable.scenarioId}" não existe mais. Edite o cenário da mesa para selecionar outro.`,
                "warning",
              );
              // Não alteramos o scenarioId automaticamente - o GM deve escolher outro
            }
          } catch (error) {
            console.error("Erro ao validar cenário:", error);
          }
        }
      };
      validateScenario();
  ...
  const handleScenarioSaved = (newId) => {
    notifyTablesUpdated();
    if (selectedTable?._id === newId) {
  ...
  const handleDeleteScenario = async () => {
    ...
    try {
      setLoading(true);
      await ScenarioService.deleteScenario(scenarioToDelete);

      // Limpar cache do scenario registry
      if (typeof window !== "undefined" && window.clearScenarioCache) {
        window.clearScenarioCache();
      }

      showNotification(
        `Cenário "${scenarioToDelete}" excluído com sucesso!`,
        "success",
      );

      // Se o cenário excluído é o atual da mesa, reverter para padrão
      if (scenarioId === scenarioToDelete) {
        setScenarioId("solo-leveling");
        // Notificar que a mesa será atualizada
        await handleUpdate();
      }

      // Atualizar lista de cenários se houver um refresh (pode ser feito via store)
      setScenarioDeleteDialogOpen(false);
      setScenarioToDelete(null);
    } catch (error) {
      console.error("Erro ao excluir cenário:", error);
      showNotification("Erro ao excluir cenário", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
                    ...
                    <Box sx={{display: "flex", gap: 1, mt: 1}}>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          setScenarioDeleteDialogOpen(true);
                        }}
                      >
                        Excluir Cenário
                      </Button>
                  ...
                  >
                    ...
                    >
                      {friendData.armas?.length > 0
                        ? friendData.armas.slice(0, 2).map((w, i) => (
                            <Box
                              key={w.id || w.name || i}
                              sx={{
                                display: "flex",
      ...
      <ScenarioAdminModal
        ...
        initialScenarioId={scenarioAdminInitialId}
        onScenarioCreated={handleScenarioSaved}
      />

      {/* Dialog de Confirmação de Exclusão de Cenário */}
      <Dialog
        open={scenarioDeleteDialogOpen}
        onClose={() => setScenarioDeleteDialogOpen(false)}
        maxWidth="xs"
        fullWidth
      >
        ...
        <DialogContent>
          <DialogContentText>
            Tem certeza que deseja excluir o cenário "{scenarioToDelete}"?
            <br />
            Esta ação <strong>não pode ser desfeita</strong> e afetará todas as
            mesas que usam este cenário.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button
            onClick={() => setScenarioDeleteDialogOpen(false)}
            color="inherit"
          >
            Cancelar
          </Button>
